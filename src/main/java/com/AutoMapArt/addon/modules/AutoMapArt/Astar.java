    ...
    public BlockPos[] findPath(BlockPos start, BlockPos goal) {
        ...
        while (!openSet.isEmpty()) {
            ...
            if (current.gScore >= maxLength || neighbors == null) {
                System.out.println("Movements Considered " + movementsConsidered + " Time taken "
        ...
        for (int dx = -1; dx <= 1; dx++) {
            for (int dz = -1; dz <= 1; dz++) {
                ...
                for (int dy = -1; dy <= 1; dy++) {
                    ...
                    if (!AutoMapArtUtils.canWalk(mc, newX, newY, newZ)) {
                        continue;
                    }

                    if (dx != 0 && dz != 0) {
                        if (!AutoMapArtUtils.canWalk(mc, node.x, newY, newZ)
                                || !AutoMapArtUtils.canWalk(mc, newX, newY, node.z))
                            continue;
                    }
