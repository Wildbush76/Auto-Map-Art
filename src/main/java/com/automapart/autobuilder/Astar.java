...
public class Astar {
    ...
    public static BlockPos[] findPath(BlockPos start, BlockPos goal, MinecraftClient mc) {
        ...
        while (!openSet.isEmpty()) {
            ...
            if (current.gScore >= MAX_LENGTH || neighbors == null) {
    ...
    private static BlockPos[] createPath(Node node) {
    ...
    private static double heuristic(Node one, Node two) {
        return Math.sqrt(
                Math.pow((double) one.x - two.x, 2) + Math.pow((double) one.y - two.y, 2) + Math.pow(one.z - two.z, 2));
    }
    ...
    private static List<Node> getNeighbors(MinecraftClient mc, Node node, BlockPos goal) {
